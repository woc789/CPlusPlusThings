...
struct  DataProcessor{
    vector<int> &data;
    DataProcessor(vector<int> &d) :data(d){

    }
    void operator()(){
        cout << "[Thread] 正在处理数据..." << endl;
        this_thread::sleep_for(chrono::seconds(10));
        long sum=0;
        try {
            sum = accumulate(data.begin(), data.end(), 0L);
        } catch (const exception& e) {
            cerr << "[Thread ERROR] 访问数据时发生错误: " << e.what() << endl;
            return;
        }
        if(sum > 0){
            cout << "[Thread] 数据处理完毕，总和: " << sum << endl;
        } else {
            cout << "[Thread] 数据总和为零，抛出内部异常..." << endl;
            throw runtime_error("Data sum is zero!");
        }
    }
};

void safe_thread_functions(){
    vector<int> local_data ={1,2,3,4,5,6};

    DataProcessor processer(local_data);
    
    thread worker_thread(processer);

    thread_guard g(worker_thread);

    cout << "[Main] 线程已启动，执行其他任务..." << endl;

    this_thread::sleep_for(chrono::milliseconds(10));  
    
    cout << "[Main] 函数即将退出，thread_guard 正在等待线程..." << endl;
}

int main(){
    try {
        safe_thread_functions();
    } catch (const exception& e) {
        cerr << "[Global Catch] 捕获到来自线程的异常: " << e.what() << endl;
    }
